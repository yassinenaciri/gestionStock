...
export const Paiement = (props: RouteComponentProps<{ url: string }>) => {
  ...
  const entity = useAppSelector(state => state.paiement.entity);
      ...
      <div className="table-responsive">
        ...
        >
          {paiementList && paiementList.length > 0 ? (
              ...
              <tbody>
                {paiementList.map((paiement, i) => (
                  <tr key={`entity-${i}`} data-cy="entityTable">
                    ...
                    <td>{paiement.mail}</td>
                    <td>{paiement.item ? <Link to={`/article-projet/${paiement.item.id}`}>{paiement.item.id}</Link> : ''}</td>
            ...
            !loading && <div className="alert alert-warning">No Paiements found</div>
          )}
        </InfiniteScroll>
      </div>
    </div>
